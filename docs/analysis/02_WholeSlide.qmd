...
```{python Load }
# mIF
IF_IMG_PATH = Path("../COLON_Xenium_data/IF_HE/IF_colon_4.ome.tif")
# XENIUM
XE_DIR = Path("../COLON_Xenium_data/xenium/")
...
```{python}
if_nucleus_geojson = Path("../COLON_Xenium_data/IF_HE/IF_colon_4_segmentation.geojson")
...
```{python}
xe_nucleus_geojson = Path(output_dir_mif, "nucleus_xenium.geojson")
# Transform coordinates
...
MATRIX_PATH = Path("../COLON_Xenium_data/IF_HE/IF_colon_4_alignment_files/matrix.csv")
# Transform coordinates
xa.module.apply_affine_transform(xe_nucleus_geojson, output_dir_mif, MATRIX_PATH, inverse = True)
```

## Validation
To validate the alignment, we compare the transformed Xenium cell segmentation against the ground-truth mIF segmentation, using Intersection over Ground Truth (IoGT, the fraction of each ground-truth cell's area recovered by the corresponding transformed cell). 
...
Note that this metric is highly sensitive to the quality and consistency of both segmentations being compared. Here, the mIF segmentation is approximate, and any region where cells are missing in one slide but not the other will bias the score. As such, IoGT should not be used as a standalone measure of alignment quality, but only to compare transformations relative to one another (e.g., sITK vs. Xenium Explorer) on the same segmentation inputs.
...
## Ground thruth
if_nucleus_geojson = Path("../COLON_Xenium_data/IF_HE/IF_colon_4_segmentation.geojson")
gdf_gt = xa.data.load_gdf_pixel_to_microns(if_nucleus_geojson, meta_source)

## sITK
transformed_inverse_10 = Path(output_dir_mif, f"transformed_inverse_{ms}.geojson")
gdf_pred = xa.data.load_gdf_pixel_to_microns(transformed_inverse_10, meta_source)

best_matches_sitk = xa.module.match_and_compute_iogt(gdf_pred, gdf_gt)
output_iogt_spatial_sitk = os.path.join(output_dir_mif, f"iogt_spatial_sitk_{ms}.png")
xa.plot.plot_spatial_alignment(best_matches_sitk, output_iogt_spatial_sitk)

## Affine
affine_transformed_inverse = Path(output_dir_mif, "affine_transformed_inverse.geojson")
...
output_iogt_spatial_affine = os.path.join(output_dir_mif, "iogt_spatial_affine.png")
xa.plot.plot_spatial_alignment(best_matches_affine, output_iogt_spatial_affine)
...
```{python Load }
# HE
HE_IMG_PATH = Path("../COLON_Xenium_data/IF_HE/HE_colon_4.ome.tif")
...
```{python Load }
meta_source, meta_target, offset_x, offset_y, proc_source, proc_target, combo_dir, combo_name = xa.data.load_images_and_metadata_he(HE_IMG_PATH, XE_DIR)
```
...
axes[1].set_title('Target (Xenium)', fontsize=10)
plt.tight_layout()
plt.savefig(f'{output_dir_he}/processed_images.png', bbox_inches='tight', pad_inches=0)
```
...
```{python}
he_nucleus_geojson = Path("../COLON_Xenium_data/IF_HE/HE_colon_4_segmentation.geojson")
...
```{python}
## Alignement file from xenium explorer
MATRIX_PATH = Path("../COLON_Xenium_data/IF_HE/HE_colon_4_alignment_files/matrix.csv")
## Transform coordinates
xa.module.apply_affine_transform(xe_nucleus_geojson, combo_dir, MATRIX_PATH, inverse = True)
```

# Validation
To validate the alignment, we compare the transformed Xenium cell segmentation against the ground-truth H&E segmentation, using Intersection over Ground Truth (IoGT, the fraction of each ground-truth cell's area recovered by the corresponding transformed cell).

We first visualize IoGT spatially across cells, then compare the distribution of IoGT and centroid distance error between the sITK (Rigid + Bspline) and Xeniume Explorer (Affine) transformations.

Note that this metric is highly sensitive to the quality and consistency of both segmentations being compared. Here, the H&E segmentation is approximate, and any region where cells are missing in one slide but not the other will bias the score. As such, IoGT should not be used as a standalone measure of alignment quality, but only to compare transformations relative to one another (e.g., sITK vs. Xenium Explorer) on the same segmentation inputs.
...
## sITK
transformed_inverse = Path(combo_dir, f"transformed_inverse_{ms}.geojson")
gdf_pred = xa.data.load_gdf_pixel_to_microns(transformed_inverse, meta_source)

best_matches_sitk = xa.module.match_and_compute_iogt(gdf_pred, gdf_gt)
output_iogt_spatial_sitk = os.path.join(combo_dir, f"iogt_spatial_sitk_{ms}.png")
xa.plot.plot_spatial_alignment(best_matches_sitk, output_iogt_spatial_sitk)

## Affine
affine_transformed_inverse = Path(combo_dir, "affine_transformed_inverse.geojson")
gdf_pred = xa.data.load_gdf_pixel_to_microns(affine_transformed_inverse, meta_source)

best_matches_affine = xa.module.match_and_compute_iogt(gdf_pred, gdf_gt)
output_iogt_spatial_affine = os.path.join(combo_dir, "iogt_spatial_affine.png")
xa.plot.plot_spatial_alignment(best_matches_affine, output_iogt_spatial_affine)
