...
In the [Registration on whole slide](analysis/02_WholeSlide.qmd) module, we provide tools to compare the results between both methods, quantifying comparable accuracy (Intersection and distance error over ground thruth ).
:::

### Installation
You can create an environment using [mamba](https://mamba.readthedocs.io/).

```bash
# Create the environment from environment.yml
mamba env create -f environment.yml
# Activate it
mamba activate xenium_align
```
...
* **Data Preparation**: Load and format images for alignment.

* **Image Registration**: Automatic alignment using SimpleITK.

* **Transformation**: Map spatial annotations (such as cell boundaries) into the target coordinate system.

* **Validation**:  Validate the result using Intersection and distance error against ground truth segmentation.

### [Exemple with a toydataset](analysis/01_ToyDataset.qmd)
